...
else 
{
	...
	if ($module == 'receive' && $act == 'search')
	{
		...
		if ($sDate != '' && $eDate != '')
		{
			if ($customerID != '')
			{
				$queryReceive = "SELECT * FROM detail_barang";
			}
			else
			{
				$queryReceive = "SELECT * FROM detail_barang";
		...
		while ($dtReceive = mysqli_fetch_array($sqlReceive))
		{
			...
			$dataReceived[] = array(	'no_detail_brg' => $dtReceive['no_detail_brg'],
									'productCode' => $dtReceive['productCode'],
									'namaSupplier' => substr($dtReceive['namaSupplier'],6),
									'mep' => $dtReceive['mep'],
									'qty_brg' => $dtReceive['qty_brg'],
									'no' => $i);
			$i++;
		}
		...
		/////////////////////////////////////////////////////////////
		 ...
		 $query = "SELECT * from detail_barang where $request=2";
		 $output=mysqli_query($connect,$query);

		 $i = 1;
		 while($dtReceive = mysqli_fetch_array($output))
 		 {
		...
		$smarty->assign("dataReceived", $dataReceived);
		...
		$smarty->assign("breadcumbTitle", "Laporan Barang Kadaluarsa");
		$smarty->assign("breadcumbTitleSmall", "Halaman untuk melihat laporan barang kadaluarsa.");
		$smarty->assign("breadcumbMenuName", "Laporan");
		$smarty->assign("breadcumbMenuSubName", "Barang Kadaluarsa");
	} 
	...
	$queryCategory = "SELECT * FROM as_categories WHERE status = 'Y' ORDER BY categoryName ASC";
		...
		$smarty->assign("msg", $_GET['msg']);
		$smarty->assign("breadcumbTitle", "Barang Kadaluarsa");
		$smarty->assign("breadcumbTitleSmall", "Halaman untuk melihat laporan barang kadaluarsa.");
		$smarty->assign("breadcumbMenuName", "Laporan");
		$smarty->assign("breadcumbMenuSubName", "Barang Kadaluarsa");
	}
