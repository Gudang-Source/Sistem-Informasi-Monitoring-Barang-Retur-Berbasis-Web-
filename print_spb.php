	...
	if ($act == 'print')
	{
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
					...
					<table cellpadding='0' cellspacing='0' style='width: 290mm;'>
						...
						if ($sDate != '' || $eDate != '')
						{
							$querySpb = "SELECT * FROM purchaseorder WHERE spbNo LIKE '%$q%' AND tanggal BETWEEN '$startDate' AND '$endDate' ORDER BY tanggal DESC";
						}
						else
						{
							$querySpb = "SELECT * FROM purchaseorder WHERE spbNo LIKE '%$q%' ORDER BY tanggal DESC";
						}
						...
						while ($dtSpb = mysqli_fetch_array($sqlSpb))
						{
							...
							$queryDetail = "SELECT * FROM detail_po WHERE spbNo = '$dtSpb[spbNo]'";
							...
							$content .= "
									<tr valign='top'>
										...
										<td style='padding: 2px 15px 2px 0px; font-size: 9pt;'>$namaSupplier</td>
										<td colspan='4'></td>
									</tr>
							";
							...
							$content .= "<tr valign='top'>
											<td colspan='7'></td>
											<td style='text-align: left;'><b>Total</b><br><br><br></td>
