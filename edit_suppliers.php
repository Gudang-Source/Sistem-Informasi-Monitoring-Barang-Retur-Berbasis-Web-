...
else 
{
	...
	if ($module == 'supplier' && $act == 'edit')
	{
		...
		$querySupplier = "SELECT * FROM supplier WHERE supplierID = '$supplierID'";
		...
		$smarty->assign("kodeSupplier", $dataSupplier['kodeSupplier']);
		$smarty->assign("namaSupplier", $dataSupplier['namaSupplier']);
		$smarty->assign("alamat", $dataSupplier['alamat']);
		$smarty->assign("noTelp", $dataSupplier['noTelp']);
	} //close bracket
